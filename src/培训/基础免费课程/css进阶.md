...
| 情况 | 背景覆盖范围 |
|------|-------------|
| `<html>` 有背景 | `<body>` 正常显示，`<body>`的背景仅覆盖 `<body>` 的边框盒 |
...
<div style="border: 1px solid red;">
  <img src="xxx.jpg">  <!-- 图片下方会出现几像素的间隙 -->
</div>
```

#### 深入原因：不仅仅是 "为字母预留空间"

表面上，这是因为图片默认 `vertical-align: baseline`，而基线下方需要为字母的 **descender（下沉部分，如 g、y、p）** 预留空间。但这里有一个更深层的问题：

> **如果 div 里只有一张图片，没有文字，为什么还会有 descender 空间？**

答案是：**只要块级容器触发了 IFC（行内格式化上下文），每一行都会存在一个不可见的匿名行内盒（anonymous inline box，常称为 strut）**。这个 strut 继承了父元素的 `font-size` 和 `line-height`，它始终参与基线对齐和行高计算，无论你是否写了可见的文字。

**对齐机制如下**：

1. **img 的基线**：对于可替换元素，基线位于图片的**底部**（margin-bottom edge）。
2. **strut 的基线**：位于其字体内容区域的底部（由 `font-size` 决定）。
3. **对齐结果**：`vertical-align: baseline` 将 img 的底部与 strut 的基线对齐。
4. **溢出的关键**：strut 的内容区域在基线下方还有 **descender 空间**。当 strut 的基线贴到 img 底部时，这部分 descender 空间会**向下溢出**，硬生生把 line box 的高度撑大了几像素。
...
img:  ┌──────────────┐
      ...
      │              │
      └──────────────┘  ← img 基线（底部）
strut:                  ← strut 基线（与 img 基线对齐）
         (descender ↓)  ← 这 3~4px 就是留白！
      ─────────────────   ← line-box 底边
```

#### 验证思路

以下任意方式都能消除留白，因为它们破坏了上述机制之一：

| 方法 | 消除留白的原理 |
|------|---------------|
| `img { display: block; }` | 让 img 脱离 IFC，不再参与基线对齐 |
| `img { vertical-align: bottom; }` | 让 img 底部与 line-box 底部对齐，而非基线 |
| `img { vertical-align: middle/top; }` | 改变对齐参考系，避开 descender 空间问题 |
| `.parent { font-size: 0; }` | strut 高度为 0，descender 空间消失 |
| `.parent { line-height: 0; }` | 同上，strut 占据的空间为 0 |

#### 解决方案

```css
/* 方案 1：改变图片的显示类型（推荐用于独占一行的图片） */
img {
  display: block;
}

/* 方案 2：改变垂直对齐方式（推荐用于与文字混排的图片） */
img {
...
> 📌 **补充**：原笔记中方案 2 提到"行块盒 (inline-block) 都不行，必须是 block"。实际上 `inline-block` 也会参与 IFC 和基线对齐，同样会产生白边。真正起作用的并非 `block` 和 `inline-block` 的区别，而是元素是否脱离了行内格式化上下文（`block` 会创建新的 BFC，不再参与 IFC）。如果只是想消除白边而不改变布局性质，`vertical-align: bottom` 往往是更精准的选择。
...
> **字体设计参考线示意图**（Consolas（2398/2048） 字体为例）：
>
> ```
>                 top
>   ────────────────────────────────────
...
> | 参考线 | 说明 |
> |--------|------|
> | **top** | 虚拟区顶部（`line-height` 撑开的最高边界） |
> | **text top / ascent / 顶线** | 字体设计中的最高可达位置 |
> | **super / 上基线** | 上标对齐参考线，穿过字母 M 腰部偏上 |
> | **baseline / 基线** | 文字基准线，字母 M 的底部 sitting 在此线上 |
> | **sub / 下基线** | 下标对齐参考线，位于 baseline 之下 |
> | **text bottom / descent / 底线** | 字体设计中的最低可达位置 |
> | **bottom** | 虚拟区底部（`line-height` 撑开的最低边界） |
> | **line gap** | `line-height` 减去 content-area 后，上下均分的空隙 |
...
> 📌 **补充**：关于 `<img>` 基线在底部却仍会触发"图片底部白边"的详细机制（strut、descender 空间、line-box 高度计算），见 [7.2 图片底部白边问题](#72-图片底部白边问题)。
...
| 3 | 常规流中非定位的**块级盒** |
| 4 | 非定位的**浮动盒** |
| 5 | 常规流中非定位的**行内盒**（包括 `inline-block`、`inline-table,同时存在的话，后写的覆盖先写的`） |
...
> 📌 **补充**：`inline-block` 和 `inline-table` 虽然在内部表现为块级/表格布局，但在外部仍属于**行内级盒**，因此与普通行内元素处于同一层。如果它们通过 `position` + `z-index`、`transform`、`opacity` 等方式创建了新的堆叠上下文，则不再遵循此规则，而是按各自的 `z-index` 值参与层叠。
